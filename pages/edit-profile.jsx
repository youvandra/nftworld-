...
const Edit_user = () => {
  ...
  const onSubmit = async (data) => {
    ...
    let image = undefined,
      banner = undefined;
    if (profilePhoto)
      await uploadFileToIPFS(profilePhoto).then(({ pinataURL }) => {
        image = pinataURL;
      });
    if (coverePhoto)
      await uploadFileToIPFS(coverePhoto).then(({ pinataURL }) => {
        banner = pinataURL;
      });
    axios
      ...
      .then(() => {
        router.push(`/user/${currentDetails.address}`);
      });
    console.log({ data, profilePhoto, coverePhoto });
  };
  ...
  const getUser = async () => {
    ...
    setValue("instagramName", user.instagramName);
  ...
  return (
    <div>
      <Meta title="Profile || NFTWORLD | NFT Marketplace Next.js Template" />
      <div className="pt-[5.5rem] lg:pt-24">
        {/* <!-- Banner --> */}
        <div className="relative">
          <img
            src={coverPreview ? coverPreview : currentDetails?.coverPhoto}
            alt="banner"
              ...
              <div className="mb-12 md:w-1/2 md:pr-8">
                ...
                <div className="mb-6">
                  ...
                  <UserId
                    ...
                    userId={currentDetails?.address ?? ""}
                  />
                </div>
                <button
                  onClick={handleSubmit(onSubmit)}
                  disabled={isLoading || !publicKey || !currentDetails}
                  type="submit"
                  className="bg-accent disabled:bg-accent-lighter shadow-accent-volume hover:bg-accent-dark rounded-full py-3 px-8 text-center font-semibold text-white transition-all"
                >
              ...
              <div className="flex space-x-5 md:w-1/2 md:pl-8">
                <form className="shrink-0">
                  <figure className="relative inline-block">
                    <img
                      src={preview ? preview : currentDetails?.profilePhoto}
                      alt="user avatar"
                      className="dark:border-jacarta-600 object-cover h-[140px] w-[140px] rounded-xl border-[5px] border-white"
                    />
